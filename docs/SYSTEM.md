...
| `slamko_loop` | T3 | **P2 ✅ · P3a 🟢** | ✅ shipped | never-lost supervisor + XFeat relocalizer + **SE3 pose-graph backend** (GN+LM, drop-bad-edge) + weld-once + cross-session seed + continuous reloc. Validated live (V1_01 multi-submap merge; V1_02→V1_01 cross-session). **P3a:** BoW vocabulary + **inverted-index DB** for scalable place-rec (`bow.{hpp,cpp}`) — candidate retrieval, geometry still verifies. **38 gtests 0 fail** (incl. 10-test stress + 4 BoW) | `889499e`+ |
