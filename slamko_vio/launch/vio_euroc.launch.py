...
def generate_launch_description():
    ...
    klt = Node(
        ...
        parameters=[{
            ...
            'ba_use_inv_depth':   ParameterValue(LaunchConfiguration('ba_use_inv_depth'), value_type=bool),
            'klt_epipolar_px':    ParameterValue(LaunchConfiguration('klt_epipolar_px'), value_type=float),
            ...
            'estimate_gravity':      ParameterValue(LaunchConfiguration('estimate_gravity'), value_type=bool),
            'submap_dump_dir':       LaunchConfiguration('submap_dump_dir'),
            'kf_per_submap':         ParameterValue(LaunchConfiguration('kf_per_submap'), value_type=int),
            'submap_seal_metres':    ParameterValue(LaunchConfiguration('submap_seal_metres'), value_type=float),
            ...
            'use_session_graph':              _bool('use_session_graph'),
            ...
            'reloc_use_vpr':            ParameterValue(LaunchConfiguration('reloc_use_vpr'), value_type=bool),
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument('ba_use_inv_depth', default_value='true',
            description='Anchored inverse-depth landmarks (OV2SLAM). NOTE: with use_inv_depth=true the window deletes any landmark whose host KF leaves the window (re-anchoring disabled after a 2026-05-18 regression), which empties the map and stalls the VI-BA in steady state (health-trace ba_fail=2). Set false for RTABmap-style world-point landmarks that survive window slides.'),
        DeclareLaunchArgument('klt_epipolar_px', default_value='0.0',
            description='Epipolar (fundamental-matrix) RANSAC gate on prev->curr KLT flow, max epipolar distance in px (0=off). Rejects drifted/mis-tracked features before stereo+PnP — for fast-motion/blur/repetitive scenes where PnP-RANSAC alone lets coherent mis-track clusters through (EuRoC V1_03/V2_03 inlier_ratio 0.2-0.4). Auto-skips when it would reject >half (degenerate F).'),
        ...
        DeclareLaunchArgument('estimate_gravity', default_value='false',
            description='P0: refine gravity direction as a 2-DOF state (S²) instead of freezing init'),
        DeclareLaunchArgument('submap_dump_dir', default_value='',
            description='Lifelong submap dump dir: seal+write submap_<id>.smap every kf_per_submap KFs'),
        DeclareLaunchArgument('kf_per_submap', default_value='25'),
        DeclareLaunchArgument('submap_seal_metres', default_value='0.0'),
        ...
        DeclareLaunchArgument('feature_source', default_value='xfeat',
            description='Detector backend: shitomasi | xfeat (xfeat = lifelong default: '
                        'carries 64-D descriptors per landmark for loop closure / reloc)'),
        ...
        DeclareLaunchArgument('use_session_graph', default_value='false',
            description='Refactor P2: async SessionGraph (full-graph VI-BA thread). VIO publishes KFs as inserted; corrections smoothly applied to live worldPose. Off by default.'),
        ...
        DeclareLaunchArgument('reloc_use_vpr', default_value='false',
            description='Enable EigenPlaces per-KF VPR descriptor (512-D) — the loop-closure recall layer'),
